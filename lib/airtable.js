...
export async function getWinner() {
  try {
    const records = await fetchTable(
      'Stances',
      'sort[0][field]=submitted_at&sort[0][direction]=desc&maxRecords=20'
    );
    const winner = records.find(
      (r) =>
        r.fields.is_winner === 1 ||
        r.fields.is_winner === true ||
        r.fields.is_winner === '1' ||
        Number(r.fields.is_winner) === 1
    );
    return winner?.fields || null;
  } catch (e) {
